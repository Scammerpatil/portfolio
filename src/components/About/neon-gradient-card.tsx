...
const NeonGradientCard: React.FC<NeonGradientCardProps> = ({
  ...
  const [convertedColor, setConvertedColor] = useState(neonColors.secondColor);

  function oklchToHex(oklchString: string) {
    const oklch = `${oklchString}`;
  ...
  useEffect(() => {
    ...
    const secondaryColor = getComputedStyle(root)
      ...
      .trim();
    setConvertedColor(oklchToHex(secondaryColor));
  ...
  return (
    ...
    >
      <div
        className={cn(
          "relative h-full min-h-[inherit] w-full rounded-[var(--card-content-radius)] bg-base-100",
